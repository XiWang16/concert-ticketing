...
const SeatMap: React.FC<SeatMapProps> = ({ eventId, eventName, date, image }) => {
                ...
                {seats
                  ...
                  .map(seat => (
                    <button
                      key={seat.id}
                      className={cn(
                        "w-8 h-8 text-xs rounded-md flex items-center justify-center transition-all duration-300",
                        seat.status === 'available' && "bg-secondary hover:bg-primary/20 cursor-pointer",
                        seat.status === 'selected' && "bg-primary text-white hover:bg-primary/80",
                        seat.status === 'reserved' && "bg-accent cursor-not-allowed",
                        seat.status === 'unavailable' && "bg-muted cursor-not-allowed opacity-50"
                      )}
        ...
        <div className="bg-muted/30 rounded-lg p-6 border border-border">
          ...
          <div className="divide-y divide-border">
            {selectedSeats.map(seat => (
              <div key={seat.id} className="py-3 flex justify-between items-center">
                <div>
